...
def _binary_search_between(array: Array[Item], item: Item, lower_index: int, upper_index: int) -> Optional[int]:
    ...
    middle_item = array.get_at(middle_index)
    if item < middle_item:
        return _binary_search_between(array, item, lower_index, middle_index - 1)
    if item == middle_item:
        return middle_index
    if item > middle_item:
        return _binary_search_between(array, item, middle_index + 1, upper_index)
