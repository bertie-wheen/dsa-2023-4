...
class _BinarySearchSubtree(Base[Key, Value]):
    ...
    def get_subtree(self, key: Key) -> "_BinarySearchSubtree[Key, Value]":
        ...
        if key == self._key:
            return self
        if key < self._key and self.has_left():
            return self._left.get_subtree(key)
        if key > self._key and self.has_right():
    ...
    def insert(self, key: Key, new_value: Value) -> None:
        ...
        if key < self._key:
            if self.has_left():
                return self._left.insert(key, new_value)
            self._left = _BinarySearchSubtree(self._tree, key, new_value, parent=self)
            self._tree._length += 1
            return
        if key > self._key:
            if self.has_right():
                return self._right.insert(key, new_value)
            self._right = _BinarySearchSubtree(self._tree, key, new_value, parent=self)
            self._tree._length += 1
            return
