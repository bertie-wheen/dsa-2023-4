...
class DoublyLinkedList(Base[Item]):
    ...
    def insert_at(self, index: int, new_item: Item) -> None:
        ...
        if self.is_empty():
            ...
            self._last_node.insert_next(new_item)
        else:
            next_node = self.get_node_at(index)
            next_node.insert_previous(new_item)
...
class DoublyLinkedNode(Base[Item]):
    ...
    def insert_previous(self, new_previous_item: Item) -> None:
        ...
        """
        old_previous_node = self._previous_node
        new_previous_node = DoublyLinkedNode(
            self._list,
            new_previous_item,
    ...
    def insert_next(self, new_next_item: Item) -> None:
        ...
        new_next_node = DoublyLinkedNode(
            self._list,
            new_next_item,
            previous_node=self,
        ...
        if self.is_last():
            ...
            old_next_node._previous_node = new_next_node
    ...
    def remove_previous(self) -> Item:
        ...
        if self.is_first():
            raise ValueError
        return self._previous_node.remove()
    ...
    def remove(self) -> Item:
        ...
        if self.is_first():
            ...
            self._previous_node._next_node = self._next_node
        if self.is_last():
            self._list._last_node = self._previous_node
        else:
            self._next_node._previous_node = self._previous_node
        self._list._length -= 1
        return self._item
