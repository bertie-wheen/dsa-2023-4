...
class StaticArrayList(Base[Item]):
    ...
    def insert_at(self, index: int, new_item: Item) -> None:
        ...
        for i in range(index):
            item = old_array.get_at(i)
            new_array.set_at(i, item)
        new_array.set_at(index, new_item)
        for i in range(index, old_length):
            item = old_array.get_at(i)
            new_array.set_at(i + 1, item)
    ...
    def remove_at(self, index: int) -> Item:
        ...
        for i in range(index):
            item = old_array.get_at(i)
            new_array.set_at(i, item)
        for i in range(index + 1, old_length):
            item = old_array.get_at(i)
            new_array.set_at(i - 1, item)
        self._array = new_array
        return old_array.get_at(index)
