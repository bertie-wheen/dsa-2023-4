...
class SinglyLinkedList(Base[Item]):
    ...
    def insert_at(self, index: int, new_item: Item) -> None:
        ...
        if self.is_empty():
            if index != 0:
                raise IndexError
            self._make_singleton(new_item)
        elif index == 0:
            self._first_node.insert_previous(new_item)
        else:
            previous_node = self.get_node_at(index - 1)
            previous_node.insert_next(new_item)
    ...
    def remove_at(self, index: int) -> Item:
        ...
        if index == 0:
            return self._first_node.remove()
        previous_node = self.get_node_at(index - 1)
        return previous_node.remove_next()
    ...
    def reverse_nodes_iterator(self) -> Iterator["SinglyLinkedNode[Item]"]:
        ...
        for item in self.iterator():
            reversed_list.insert_first(item)
        return reversed_list.nodes_iterator()
        ...
        # note that the O(self.get_length()) space of the first approach is somewhat - at least asymptotically -
        # mitigated by the fact that this list already requires that much space, so the temporary list requiring that
        # much space again only actually increases the total space usage by factor of 2 and therefore in a sense doesn't
        # matter (unless e.g. this list already requires 2/3 of the space available, in which case only the second
        # approach would work as the first would cause an out-of-memory error)
...
class SinglyLinkedNode(Base[Item]):
    ...
    def insert_previous(self, new_previous_item: Item) -> None:
        ...
        """
        new_previous_node = SinglyLinkedNode(
            self._list,
            new_previous_item,
            next_node=self,
        )
        if self.is_first():
            self._list._first_node = new_previous_node
        else:
            old_previous_node = self.get_previous_node()
            old_previous_node._next_node = new_previous_node
    ...
    def insert_next(self, new_next_item: Item) -> None:
        ...
        """
        old_next_node = self._next_node
        new_next_node = SinglyLinkedNode(
            self._list,
            new_next_item,
            next_node=old_next_node,
        )
        if self.is_last():
            self._list._last_node = new_next_node
    ...
    def remove_previous(self) -> Item:
        ...
        if self.is_first():
            raise ValueError
        old_previous_node = self.get_previous_node()
        return old_previous_node.remove()
    ...
    def remove(self) -> Item:
        ...
        """
        if self.is_first():
            self._list._first_node = self._next_node
            self._list._length -= 1
            return self._item
        previous_node = self.get_previous_node()
        return previous_node.remove_next()
    ...
    def remove_next(self) -> Item:
        ...
        if self.is_last():
            raise ValueError
        old_next_item = self._next_node._item
        if self._next_node.is_last():
            self._list._last_node = self
        self._next_node = self._next_node._next_node
        self._list._length -= 1
        return old_next_item
