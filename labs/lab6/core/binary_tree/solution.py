...
class BinarySubtree(Base[Item]):
    ...
    def contains(self, item: Item) -> bool:
        ...
        if item == self._item:
            return True
        return self.left_contains(item) or self.right_contains(item)
    ...
    def get_level(self) -> int:
        ...
        if self.is_root():
            return 0
        return 1 + self._parent.get_level()
    ...
    def _calculate_length(self) -> int:
        ...
        :returns: the length
        """
        left_length = self.get_left_length()
        right_length = self.get_right_length()
    ...
    def pre_iterator(self) -> Iterator[Item]:
        ...
        yield self._item
        if not self.is_left_empty():
            yield from self._left.pre_iterator()
        if not self.is_right_empty():
            yield from self._right.pre_iterator()
    ...
    def post_iterator(self) -> Iterator[Item]:
        ...
        """
        if not self.is_left_empty():
            yield from self._left.post_iterator()
        if not self.is_right_empty():
            yield from self._right.post_iterator()
        yield self._item
    ...
    def reverse_pre_iterator(self) -> Iterator[Item]:
        ...
        yield self._item
        if not self.is_right_empty():
            yield from self._right.reverse_pre_iterator()
        if not self.is_left_empty():
            yield from self._left.reverse_pre_iterator()
    ...
    def reverse_iterator(self) -> Iterator[Item]:
        ...
        """
        if not self.is_right_empty():
            yield from self._right.reverse_iterator()
        yield self._item
        if not self.is_left_empty():
            yield from self._left.reverse_iterator()
    ...
    def reverse_post_iterator(self) -> Iterator[Item]:
        ...
        """
        if not self.is_right_empty():
            yield from self._right.reverse_post_iterator()
        if not self.is_left_empty():
            yield from self._left.reverse_post_iterator()
        yield self._item
