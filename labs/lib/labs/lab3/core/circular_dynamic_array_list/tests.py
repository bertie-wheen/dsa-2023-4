...
from lab3.core.circular_dynamic_array_list.exercise import CircularDynamicArrayList
...
def resize_then_get_capacity(
    ...
    list = CircularDynamicArrayList.build(items, capacity=old_capacity)
...
def get_at_then_remove_at(
    ...
    list = CircularDynamicArrayList.build(items)
    yield list.get_at(index)
...
def insert_at_then_get_at_earlier(
    ...
    list = CircularDynamicArrayList.build(chain(earlier_items, later_items))
...
def insert_at_when_should_not_grow(
    ...
    list = CircularDynamicArrayList.build(items, capacity=capacity)
    list.insert_at(index, item)
    yield capacity
    yield list.get_capacity()
...
def remove_at_then_get_at_earlier(
    ...
    list = CircularDynamicArrayList.build(chain(earlier_items, (item,), later_items))
    early_item = list.get_at(early_index)
