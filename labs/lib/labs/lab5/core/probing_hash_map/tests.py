...
from lab5.core.probing_hash_map.exercise import ProbingHashMap
...
def contains(
    ...
    map = ProbingHashMap.build(mappings.iterator())
...
def contains_not(
    ...
    map = ProbingHashMap.build(mappings)
    yield False
    yield map.contains(key)
...
@Test
def insert_then_get_capacity(
    mappings: Array[tuple[Key, Value]],
    max_load_factor_component: Annotated[int, GE(1), LE(4)],
    key: Key,
    value: Value,
):
    max_load_factor = (max_load_factor_component + 4) / 8
    map = ProbingHashMap.build(mappings.iterator(), max_load_factor=max_load_factor)
    old_capacity = map.get_capacity()
    map.insert(key, value)
    if map.get_length() > old_capacity * max_load_factor:
        new_capacity = old_capacity * 2
    else:
        new_capacity = old_capacity
    yield new_capacity
    yield map.get_capacity()
...
def remove_then_get_capacity(
    ...
    old_capacity = map.get_capacity()
    map.remove(key)
    if map.get_length() < max_load_factor * old_capacity / 4:
